...
import { ChartHoverData, ChartModel, ChartModelParams } from 'components/ChartsV2/ChartModel'
import { LiquidityBarSeries } from 'components/ChartsV2/LiquidityChart/liquidity-bar-series'
import {
  LiquidityBarData,
  LiquidityBarProps,
  LiquidityBarSeriesOptions,
} from 'components/ChartsV2/LiquidityChart/renderer'
...
async function calculateActiveRangeTokensLocked(
  ...
  try {
    ...
    const amount0Locked = parseFloat(tick.price0)
    ...
    const amount1Locked = parseFloat(tick.price0)
  ...
  useEffect(() => {
    async function formatData() {
      ...
      for (let index = 0; index < ticksProcessed.length; index++) {
        ...
        let price0 = t.price0 as any
        let price1 = t.price0 as any

        if (isActive && activePoolData.activeTick && activePoolData.activeTick) {
          activeRangeIndex = index
          activeRangePercentage = (activePoolData.activeTick - t.tick) / TICK_SPACINGS[feeTier]
        ...
        barData.push({
          ...
          price0: formatNumber(price0, NumberType.ChartFiatValue) || '0',
          price1: formatNumber(price1, NumberType.ChartFiatValue) || '0',
          time: fakeTime,
          amount0Locked,
          amount1Locked,
        })
    ...
    formatData()
  }, [activePoolData, tokenA, tokenB, formatNumber, isReversed, feeTier])
