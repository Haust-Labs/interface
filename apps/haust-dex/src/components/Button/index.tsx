...
export const ButtonPrimary = styled(BaseButton)`
  ...
  &:focus {
    ...
    color: ${({ theme }) => theme.buttonPrimaryTextHover};
  }
  &:hover {
    background: ${({ theme }) => theme.buttonPrimaryHover};
    color: ${({ theme }) => theme.buttonPrimaryTextHover};
  ...
  &:disabled {
    background: ${({ theme, altDisabledStyle, disabled }) =>
      altDisabledStyle ? (disabled ? theme.buttonDisabled : theme.buttonDisabled) : theme.buttonDisabled};
    color: ${({ altDisabledStyle, disabled, theme }) =>
      altDisabledStyle ? (disabled ? theme.buttonTextDisabled : theme.buttonTextDisabled) : theme.buttonTextDisabled};
    cursor: auto;
    box-shadow: none;
...
export function ButtonDropdownLight({ disabled = false, children, ...rest }: { disabled?: boolean } & ButtonProps) {
  return (
    <ButtonOutlined {...rest} disabled={disabled}>
      <RowBetween>
        <div style={{ display: 'flex', alignItems: 'center' }}>{children}</div>
        <ChevronDown size={24} color={colors.neutralDark} />
      </RowBetween>
    </ButtonOutlined>
  )
}
