...
import {POOL_DEPLOYER_ADDRESSES} from 'constants/addresses'
...
const DEFAULT_CHAINS = [SupportedChainId.HAUST, SupportedChainId.HAUST_TESTNET]
  ...
  const fetchPositionInfo = useCallback(
    async (positionDetails: PositionDetails[], chainId: SupportedChainId, multicall: UniswapInterfaceMulticall) => {
      ...
      positionDetails.forEach((details) => {
        ...
        if (!poolAddress) {
          const factoryAddress = POOL_DEPLOYER_ADDRESSES[chainId]
