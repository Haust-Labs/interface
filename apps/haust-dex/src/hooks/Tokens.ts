...
export function useUnsupportedTokens(): { [address: string]: Token } {
  ...
  const l2InferredBlockedTokens: typeof unsupportedTokens = useMemo(() => {
    ...
    return list.tokens.reduce((acc, tokenInfo) => {
      ...
      if (
        bridgeInfo &&
        bridgeInfo[SupportedChainId.HAUST] &&
        bridgeInfo[SupportedChainId.HAUST].tokenAddress &&
        unsupportedSet.has(bridgeInfo[SupportedChainId.HAUST].tokenAddress)
      ) {
        const address = bridgeInfo[SupportedChainId.HAUST].tokenAddress
        // don't rely on decimals--it's possible that a token could be bridged w/ different decimals on the L2
        return { ...acc, [address]: new Token(SupportedChainId.HAUST, address, tokenInfo.decimals) }
      }
      return acc
    }, {})
