...
const dayQuery = gql`
  query TokenVolumeChartDay($token: String!, $dateGte: Int!, $first: Int!) {
    tokenDayDatas(
...
export default function useTokenVolumeChart(
  ...
  return useMemo(
    () => ({
      ...
      data: transformedData as TokenVolumeChartQuery,
    }),
