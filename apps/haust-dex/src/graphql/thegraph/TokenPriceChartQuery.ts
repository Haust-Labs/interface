...
const dayQuery = gql`
  query TokenPriceChartDay($token: String!, $dateGte: Int!, $first: Int!) {
    tokenDayDatas(
...
export default function useTokenPriceChart(
  ...
  return useMemo(
    () => ({
      ...
      data: transformedData as TokenPriceChartQuery,
    }),
